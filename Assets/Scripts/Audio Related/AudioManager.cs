...
/// <summary>
/// Contains all the logic to playback audio.
/// </summary>
public class AudioManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (Instance != null)
        {
            Debug.LogWarning("There's more than one AudioManager! Destroying the new one.");
            Destroy(gameObject);
            return;
        }
    ...
    #region Audio Listener Management Methods
    ...
    public void PlayOneshotOnTarget(BasicSound basicSound, Transform target)
    {

        if (!SoundShouldPlay(basicSound)) { return; }

        PlayOneshot(basicSound, GetAudioSourceFromTarget(target, false));
    ...
    /// </summary>
    /// <param name="soundBase">The SoundBase to be started at a location.</param>
    /// <param name="position">The position for the SoundContainer to be played.</param>
    /// <param name="objCalledFrom">The object name calling this function. Used for debug purposes.</param>
    public void PlaySoundBaseAtPos(SoundBase soundBase, Vector2 position, string objCalledFrom)
    {

        if (!SoundShouldPlay(soundBase)) { return; }

        GameObject audioSourceGameObject = new GameObject();
        audioSourceGameObject.transform.name = $"Playing {soundBase.name} at {position}. Created by {objCalledFrom}"; //can be deleted for final builds
    ...
    private void PlaySoundBaseOnAudioSource(SoundBase soundBase, AudioSource audioSource)
    {
        ...
        switch (soundBase.GetSoundType())
        {
            case SoundType.BasicSound:
                {
                    ...
                    BasicSound basicSound = (BasicSound)soundBase;
                    PlayBasicSound(basicSound, audioSource);
            ...
            case SoundType.SoundContainer:
                {
                    ...
                    SoundContainer soundContainer = (SoundContainer)soundBase;
            ...
            default:
                {
                    print("Default case in AudioManager.PlaySoundBaseOnAudioSource reached!\nSoundBases can only be of type 'BasicSound' (0) or 'SoundContainer' (1)");
                    break;
                }
        }
    }

    private void PlayBasicSound(BasicSound sound, AudioSource audioSource)
    {
        ...
        if (sound.audioClip != null)
            ...
            print($"Tried to play a null sound on {audioSource.gameObject.name}.");
    ...
    private IEnumerator PlaySoundContainer(SoundContainer soundContainer, AudioSource audioSource)
    {
        ...
        if (soundsLength < 1)
        {
            print($"Tried to play a sound container ({soundContainer.name}) on {audioSource.gameObject.name}. Less than 1 sound in container.");
            yield break;
        }
    ...
    public IEnumerator FadeMusic(AudioSource audioSourceToFade, float fadeDuration, float targetVolume, bool stopOnEnd)
    {
        ...
        {
            audioSourceToFade.volume = Mathf.Lerp(audioSourceToFade.volume, targetVolume, timeElapsed / fadeDuration);
            timeElapsed += Time.fixedDeltaTime;
            yield return null;
        }

        if (stopOnEnd && timeElapsed < fadeDuration)
        {
    ...
    public IEnumerator FadeToDestroyCoroutine(AudioSource audioSourceToFade, float startValue, float duration, bool destroyObject)
    {
    ...
    public void ApplySoundSettingsToSound(SoundBase soundToCopySettingsFrom, SoundBase soundToApplySettingsTo)
    {

        if (!SoundShouldPlay(soundToCopySettingsFrom) || !SoundShouldPlay(soundToApplySettingsTo)) { return; }

        soundToApplySettingsTo.outputAudioMixerGroup = soundToCopySettingsFrom.outputAudioMixerGroup;
        soundToApplySettingsTo.soundSettings = soundToCopySettingsFrom.soundSettings;
    }
